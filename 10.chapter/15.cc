...
int main(int argc, char **argv)
{
  ...
  while(cin >> author)
  {
    pair<map<string, string>::iterator, map<string, string>::iterator> pos = bookmap.equal_range(author); 
    if(pos.first != pos.second)
    {
      cout << "find books with author " << author << endl; 
      for(multimap<string, string>::iterator it = pos.first; it != pos.second; ++it)
        cout << it->second << endl; 

      bookmap.erase(author); 
    }
    else 
      cout << "not find." << endl; 
