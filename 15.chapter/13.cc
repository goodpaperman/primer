...
public:
  ...
  { cout << "WordQuery constructed by " << word << endl; }
private:
...
public:
  ...
  { cout << "Query constructed by " << word  << endl; }
  Query(const Query &c) : q(c.q), use(c.use) 
  { ++ *use; 
    cout << "Query copy-constructed by "; 
    c.display(cout); 
    cout  << endl; 
  }
  ~Query() { decr_use(); }
  Query& operator=(const Query& c){
    cout << "Query assignment" << endl; 
    ++ *c.use; 
    decr_use(); 
    q = c.q; 
    use = c.use; 
  ...
  ostream& display(ostream& o) const {return q->display(o); }

private:
  Query(Query_base* query) : q(query), use(new int(1)) 
  { cout << "Query constructed by "; query->display(cout); cout  << endl; } 
...
class NotQuery : public Query_base{
  ...
  { cout << "NotQuery constructed by" << query << endl; }
  virtual set<line_no> eval(const TextQuery &tq) const {
    set<line_no> ret; 
...
protected:
  ...
  { cout << "BinaryQuery constructed by " << lhs << " and " << rhs << endl; }
...
class OrQuery : public BinaryQuery{
  ...
  { cout << "OrQuery constructed by " << lhs << " and " << rhs << endl; }
  virtual set<line_no> eval(const TextQuery &tq) const {
...
class AndQuery : public BinaryQuery{
  ...
  { cout << "AndQuery constructed by " << lhs << " and " << rhs  << endl; }
  virtual set<line_no> eval(const TextQuery &tq) const {
    set<line_no> ret; 
