...
private:
  virtual set<line_no> eval(const TextQuery& tq) const{
    cout << "WordQuery eval for " << query_word << endl; 
...
public:
  ...
  set<TextQuery::line_no> eval(const TextQuery& t) const {
    cout << "Query eval for "; display(cout);  cout  << endl; 
    return q->eval(t); 
  }
...
class OrQuery : public BinaryQuery{
  ...
  {  }
  virtual set<line_no> eval(const TextQuery &tq) const {
    cout << "OrQuery eval for " << left << " and " << right << endl; 
...
class AndQuery : public BinaryQuery{
  ...
  {  }
  virtual set<line_no> eval(const TextQuery &tq) const {
    cout << "AndQuery eval for " << left << " and " << right << endl; 
    set<line_no> ret; 
